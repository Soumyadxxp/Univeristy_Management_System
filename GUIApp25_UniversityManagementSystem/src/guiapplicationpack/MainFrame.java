...
public class MainFrame extends JFrame
{
    ...
    public MainFrame()
    {
        ...
        for(int i = 0;i<7;i++)
        {
            ...
            adminItems[i] = makeMenuItem(adminItemCap[i],menuAdmin,"right.gif");
        }
        for(int i = 0;i<4;i++)
        {
            if(i == 3) menuProfessor.addSeparator();
            profItems[i] = makeMenuItem(profItemCap[i],menuProfessor,"right.gif");
        }
        for(int i = 0;i<3;i++)
        {
            if(i == 2) menuStudent.addSeparator();
            stuItems[i] = makeMenuItem(stuItemCap[i],menuStudent,"right.gif");
        }
    }
}
